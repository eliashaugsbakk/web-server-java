...
- Configure the url and authentication once. (Should persist)
...
To beguine with, a manually distributed authentication token will be used to authenticate the client.
Later, username + passphrase will be used. (Could also include a 2FA like TOTP)
The client will send the data to the endpoint: server-url/upload/.
The data will be sent as a JSON file containing preprocessed data (Normalized and compressed images + HTML).
...
- Image files
  ...
  compression or bundling will not be needed for the time being.

A GUI could be implemented in the future, probably using JavaFX.

Would a fingerprint of the uploaded content be useful for the server to validate the data?
...
- The program should validate that the images referenced in the Markdown file is also being uploaded.
...
- Auth token could be sent as an http header.
...
```
The order of the flags would probably not matter, and the distinction between Markdown and images would probably be done by reading the file extentions.
The setting flags (--url, --port, --token) would be stored in between sessions in .config/webServerUploader
The port would be the port towards the local tor daemon, likely run by Tor-daemon or the Tor Browser.
File pats could be relative from where the program is run, or an absolute path.
